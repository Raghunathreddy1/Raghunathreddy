...
public class PatientPersonalDetailsService {
	...
	/*
	 * public PatientPersonalDetails updatePatientPersonalDetails(int pId,
	 * PatientPersonalDetails patient) { Optional<PatientPersonalDetails> optional =
	 * patientDao.findById(pId); if (optional.isPresent()) { PatientPersonalDetails
	 * dbPatient = optional.get(); dbPatient.setpName(patient.getpName());
	 * dbPatient.setpAge(patient.getpAge());
	 * dbPatient.setSymptoms(patient.getSymptoms()); patientDao.save(dbPatient); }
	 * return null; }
	 */
